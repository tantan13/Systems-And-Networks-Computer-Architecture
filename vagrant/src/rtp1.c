...
static packet_t *packetize(char *buffer, int length, int *count) {
     ...
     */
    int packetLength = length / MAX_PAYLOAD_LENGTH;
    int lastPacketLength = length % MAX_PAYLOAD_LENGTH;
    if (lastPacketLength != 0) {
        packetLength++;
    } 
    packet_t *packets = malloc(sizeof(packet_t) * packetLength);
    for (int i = 0; i < packetLength; i++) {
        if (i == packetLength - 1) {
            packets[i].payload_length = lastPacketLength;
        ...
        memcpy(packets[i].payload, &buffer[MAX_PAYLOAD_LENGTH * i], packets[i].payload_length);
    }
    *count = packetLength;
...
static void *rtp_recv_thread(void *void_ptr) {
    ...
    do {
        ...
        do {
            ...
            */
            //correctSum = checksum(packet.payload, packet.payload_length);
            ...
            if ((packet.type == DATA || packet.type == LAST_DATA)) {
                ...
                net_send_packet(connection->net_connection_handle, &currPacket);
            } else {
                pthread_mutex_lock(&connection->ack_mutex);
                packet_t currPacket;
                //currPacket.type = DATA;
                if (currPacket.type == NACK) {
                    connection->check = 1;
                    connection->ack_type = NACK;
                    pthread_cond_signal(&connection->ack_cond);
                } else if (currPacket.type == ACK) {
                    connection->ack_type = ACK;
                    connection->check = 1;
                    pthread_cond_signal(&connection->ack_cond);
                }
            ...
            // if ((packet.type == DATA || packet.type == LAST_DATA) && !(checksum(packet.payload, packet.payload_length) == packet.checksum)) {
            //     //NACK appropriate
            //     printf("%s\n", packet.payload);
            //     currPacket.type = NACK;
            //     net_send_packet(connection->net_connection_handle, &currPacket);
            //     packet.type = DATA;
            // } else if ((packet.type = DATA || packet.type == LAST_DATA) && checksum(packet.payload, packet.payload_length) == packet.checksum) {
            //     //ACK appropriate
            //     printf("%s\n", packet.payload);
            //     currPacket.type = ACK;
            //     net_send_packet(connection->net_connection_handle, &currPacket);
...
static void *rtp_send_thread(void *void_ptr) {
    ...
    do {
        ...
        for (i = 0; i < array_length; i++) {
            ...
            //packet_t currPacket;
            //ccurrPacket.type = DATA;
            pthread_mutex_lock(&connection->ack_mutex);
            while (connection->check == 0) {
                pthread_cond_wait(&connection->ack_cond, &connection->ack_mutex); //wait for the ack?
            }
            if (connection->ack_type == NACK) {
                //resend last packet
                connection->check = 0;
                i--;
            } else if (connection->ack_type == ACK) {
                connection->check = 0;
            }
            pthread_mutex_unlock(&connection->ack_mutex);
        }
