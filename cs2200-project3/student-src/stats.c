...
void compute_stats() {
	...
	stats.aat = ((((double)stats.reads/(double)stats.accesses) * (double)(MEMORY_READ_TIME)) 
	+ (((double)stats.writes/(double)stats.accesses) * (double)(MEMORY_READ_TIME))
	+ (((double)stats.page_faults/(double)stats.accesses) * (double)(DISK_PAGE_READ_TIME)) 
	+ (((double)stats.writebacks/(double)stats.accesses) * (double)(DISK_PAGE_WRITE_TIME)));
}
